...
function preload() {
  rockPaperScissorsShader = loadShader(
...
function setup() {
  ...
  rockColour = toRGBArray(
    window.tinycolor({ h: hue + 90, s: 0.55, l: 0.25 }).toRgb()
  );
  paperColour = toRGBArray(
    window.tinycolor({ h: hue, s: 0.75, l: 0.5 }).toRgb()
  );
  scissorsColour = toRGBArray(
    window.tinycolor({ h: hue - 90, s: 0.95, l: 0.75 }).toRgb()
  );
}
...
function draw() {
  ...
  rockPaperScissorsShader.setUniform('canvasSize', [
    latticeWidth,
    latticeHeight,
  ]);
  rockPaperScissorsShader.setUniform(
    'rockColour',
    rockColour.map((v) => v / 255)
  );
  rockPaperScissorsShader.setUniform(
    'paperColour',
    paperColour.map((v) => v / 255)
  );
  rockPaperScissorsShader.setUniform(
    'scissorsColour',
    scissorsColour.map((v) => v / 255)
  );
