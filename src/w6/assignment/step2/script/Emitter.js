class Emitter {
  ...
  createBall(emittingPosX, emittingPosY) {
    for (let i = 0; i < 360; i++) {
      // 계산된 방향을 조정하여 원 모양으로 파티클을 배치합니다.
...
//       // 계산된 방향을 조정하여 원 모양으로 파티클을 배치합니다.
//       const angle = (TAU / 360) * i;
//       const x = emittingPosX + cos(angle) * 100; // 100은 원의 반지름입니다.
//       const y = emittingPosY + sin(angle) * 100; // 100은 원의 반지름입니다.
...
// function mouseClicked() {
//   // 마우스 클릭 지점을 중심으로 360도 방향으로 파티클을 생성합니다.
//   emitter.createBall(mouseX, mouseY);
// }
